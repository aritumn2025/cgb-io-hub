...
- Hub が配布する Web ページからの UI 操作（タッチスティック＋ A/B/Start/Back）
...
- **MVP**: `axisX/axisY/btnA/btnB/btnStart/btnBack` を自前の変数に反映し、移動・アクションへ適用
...
- `InputState`（axes{x,y}, btn{a,b,start,back} _→ 確定後に拡張_ , t: UnixMillis）
...
type InputState struct {
    ...
    Btn  struct{ A, B, Start, Back bool }
    T    int64 // unix ms
}
