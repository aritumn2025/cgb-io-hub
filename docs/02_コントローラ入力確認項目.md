...
- [ ] 別ターミナルで Game 役として websocat を接続し、
  ...
  ```bash
  websocat -vE ws://localhost:8765/ws
  {"role":"game"}
  ```
...
- [ ] コントローラをタップ・ドラッグしていない状態で `axes.x` / `axes.y`
  ...
  ```json
  {
    ...
    "id": "p1",
    "axes": { "x": 0, "y": 0 },
    "btn": { "a": false, "b": false, "start": false, "back": false },
    "t": 1761688445279
  }
  ```
...
- [ ] `A` ボタンをタップすると `btn.a` が `true` に変化し、離すと `false` が送信される
  ```json
  {"btn":{"a":true,"b":false,"start":false,"back":false}}
  {"btn":{"a":false,"b":false,"start":false,"back":false}}
  ```
- 条件: `pointerdown` で `true`、`pointerup`/`pointercancel` 等で `false` が送信される
- [ ] `B` ボタンでも同様に `btn.b` が `true` → `false` と変化する
- 条件: `B` ボタンの押下／解放に連動
- [ ] `Start` / `Back` ボタンでも同様の真偽値変化が確認できる
- 条件: 各ボタンの押下／解放に連動
- [ ] ボタンを押しっぱなしにすると `true` のメッセージが維持され、
      離すと `false` が送信される
- 条件: 押下中は再送時も `true`、解放時のフレームで `false` が送信される

## 複合入力確認

- [ ] スティック操作中に `A` ボタンを押しても、両方の値が同一メッセージ内で更新される
  ```json
  {
  ...
  ```
- 条件: スティック移動とボタン押下が同じ送信サイクルで発生した際のペイロード例
- [ ] 複数ボタン (`A`+`B` など) を同時に押すと、それぞれが `true` になる
- 条件: 同一フレームで複数ボタンを押下すると、該当キーが同時に `true`
